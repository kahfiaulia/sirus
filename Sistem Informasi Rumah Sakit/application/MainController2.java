...
public class MainController2<Nama> implements Initializable{
	@FXML
	private TableView<Pasien> tblPasien;
	@FXML
	private TableColumn<Pasien, String> nomor;
	@FXML
	...
	public void Refresh(ActionEvent event) throws IOException{
		...
		try {
	        ...
	        while (rs.next()) {
	        	list.add(new Pasien(rs.getString("nomor"), rs.getString("nama"), rs.getString("jeniskelamin"), rs.getInt("usia"), rs.getString("ruang"),rs.getString("penyakit"),rs.getString("dokter"), rs.getDate("tanggalmasuk")));
	        }
		}
		 catch(Exception e){
	            e.printStackTrace();
	...
	public void Ubah(ActionEvent event) throws IOException{
        ...
        try{    
            	Parent halamanUtamaParent = FXMLLoader.load(getClass().getResource("/application/ubahdata.fxml"));
    			Scene halamanUtamaScene = new Scene (halamanUtamaParent);
    			Stage app_stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
    			app_stage.setTitle("Sistem Informasi Rumah Sakit Terpadu");
    			app_stage.setScene(halamanUtamaScene);
	...
	public void Tambah(ActionEvent event) throws IOException{
	 ...
	 @FXML
		public void Cari(ActionEvent event) throws IOException{
		 	...
		 	try {
			    java.sql.Connection c = (Connection) teskoneksi.configDB();
		    	String sql = "SELECT * FROM pasiendata WHERE nama = '"+txtCari.getText() +"'";
		        java.sql.Statement s = c.createStatement();
		        ResultSet rs = s.executeQuery(sql);
		        tblPasien.getItems().clear();
		        while (rs.next()) {
	...
	public void Hapus(ActionEvent event) throws IOException, SQLException{
		...
		if (result.get() == ButtonType.OK){
			try {
			    ...
			    stmt.setString(1, selectedItem.getNomor());
			    stmt.executeUpdate();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		nomor.setCellValueFactory(new PropertyValueFactory<Pasien, String>("nomor"));
		...
		nomor.setCellFactory(TextFieldTableCell.forTableColumn());
		...
		ruang.setCellFactory(TextFieldTableCell.forTableColumn());
		penyakit.setCellFactory(TextFieldTableCell.forTableColumn());
		dokter.setCellFactory(TextFieldTableCell.forTableColumn());
		masuk.setCellFactory(TextFieldTableCell.forTableColumn(new DateStringConverter()));
